...
export default function StudentGradeForm({student, subject, classroom, show, setAddGradeShow, onComplete}) {
  const defaultForm = {
    description: "", dateTs: new Date().toISOString().substring(0, 10), grade: null, weight: 1,
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...formData, studentId: student.id, subjectId: subject.id,
    };
            ...
            <div className="d-flex flex-row gap-2">
              ...
              <Button variant="primary" type="submit" disabled={studentAddGradeCall.state === 'pending'}>
                {studentAddGradeCall.state === 'pending' ? (
                    <Icon size={0.8} path={mdiLoading} spin={true}/>) : ("Přidat")}
              </Button>
            </div>
          </div>
        </Modal.Footer>
